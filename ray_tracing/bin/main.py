...
if __name__ == "__main__":
    ...
    optim = torch.optim.Adam(
        osm.parameters(), lr=1e-5, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-3
    )
    ...
    for epoch in range(99999):
        ...
        for i in range(iter):
            ...
            if i % flash_hz == 0:
                ...
                tick = time.time()
                print(
                    f"[i loss c_t a_t], [{i:<8} {loss.item():<8.6f} {tick-tock:<8.2f} {tick-first_tick:<8.2f}  {u:<8.2f}]"
                )  # i loss cost_time all_time
            if stop == 1:
                time.sleep(5)
                stop = 0
    a = 1
